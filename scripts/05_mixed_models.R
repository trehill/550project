...
# Author: Eva MacLennan + Tessa Rehill 
...
#read in data 
data <- read.csv(here::here("Desktop", "550project", "datasets", "processed_data", "transect_final.csv"),
                 head=TRUE)

#Workflow 

#1) determine random effects 
#run with all three random effect structures with all fixed effect (ie. only Transect, only Year, and Transect + Year)
...
#Model 9: random effect: Year + Day
...
#Model 10: Random effects: Year Fixed effects: urchin presence
...
#Model 11: Random effects: Year Fixed effects: invert_richness 
...
#Model 12: Random effects: Year Fixed effects: substrate
...
#Model 13: Random effects: Year, Fixed effects: urchin presence, invert richness
...
#Model 14: Random effects: Year Fixed effects: urchin presence, substrate
...
#Model 15: Random effects: Year, Fixed effects: invert richness, substrate
...
#Model 16: Random: Year, Fixed: urchin, depth
...
#Model 17: Random:Year Fixed: urchin, depth, substrate
...
#Model 18: Random: Year Fixed: urchin, depth, substrate,invert
...
#depth 
#Model 1: Random effect: ONLY Transect 
m1 <- glmer(urchin_presence_transect ~ kelp_presence_transect + avg_depth + Substrate + invert_richness_sum + (1|Transect), 
            data = data, 
            family = binomial)
AIC1 <- AIC(m1)
...
#Model 2: Random effect: ONLY Year 
m2 <- glmer(urchin_presence_transect ~ kelp_presence_transect + avg_depth + Substrate + invert_richness_sum + (1|Year), 
            data = data, 
            family = binomial)
...
#Model 3: Random effect: ONLY Month
m3 <- glmer(urchin_presence_transect ~ kelp_presence_transect + avg_depth + Substrate + invert_richness_sum + (1|Month), 
            data = data, 
            family = binomial)

AIC3 <- AIC(m3)

#Model 4: Random effect: Transect + Year 
m4 <- glmer(urchin_presence_transect ~ kelp_presence_transect + avg_depth + Substrate + invert_richness_sum + (1|Year) + (1|Transect), 
            data = data, 
            family = binomial)

AIC4 <- AIC(m4)

#Model 5: random effect: Transect + Month 
m5 <- glmer(urchin_presence_transect ~ kelp_presence_transect + avg_depth + Substrate + invert_richness_sum +(1|Transect) + (1|Month), 
            data = data, 
            family = binomial)
...
m6 <- glmer(urchin_presence_transect ~ kelp_presence_transect + avg_depth + Substrate + invert_richness_sum +(1|Year) + (1|Month), 
            data = data, 
            family = binomial)

AIC6 <- AIC(m6)

#Model 7: random effect: Year + Day
m7 <- glmer(urchin_presence_transect ~ kelp_presence_transect + avg_depth + Substrate + invert_richness_sum +(1|Year) + (1|Day), 
            data = data, 
            family = binomial)

AIC7 <- AIC(m7)

#from this- the model with random effects with Year + Month (AIC6)

#DETERMINING FIXED EFFECTS BASED ON BEST RANDOM EFFECT STRUCTURE ####

#Model 8: Random effects: Year + Month Fixed: Kelp
m8 <- glmer(urchin_presence_transect ~ kelp_presence_transect + (1|Year) + (1|Month), 
            data = data, 
            family = binomial)
...
#Model 9: Random effects: Year, Month Fixed effects: invertebrate richness
m9 <- glmer(urchin_presence_transect ~ invert_richness_sum + (1|Year) + (1|Month), 
            data = data, 
            family = binomial)

AIC9 <- AIC(m9)

#Model 10: Random effects: Year Month Fixed effects: substrate
m10 <- glmer(urchin_presence_transect ~ Substrate + (1|Year)  + (1|Month), 
            data = data, 
            family = binomial)

AIC10 <- AIC(m10)

#Model 11: Random effects: Year, Month Fixed effects: kelp presence, invert richness
m11 <- glmer(urchin_presence_transect ~ kelp_presence_transect + invert_richness_sum + (1|Year) + (1|Month), 
             data = data, 
             family = binomial)

AIC11 <- AIC(m11)

#Model 12: Random effects: Year, Month Fixed effects: kelp presence, substrate
m12 <- glmer(urchin_presence_transect ~ kelp_presence_transect + Substrate + (1|Year)+ (1|Month), 
             data = data, 
             family = binomial)

AIC12 <- AIC(m12)

#Model 13: Random effects: Year, Month Fixed effects: invert richness, substrate
m13 <- glmer(urchin_presence_transect ~ invert_richness_sum + Substrate + (1|Year)+ (1|Month), 
             data = data, 
             family = binomial)

AIC13 <- AIC(m13)

#Model 14: Random: Year,Month, Fixed: kelp, depth
m14 <- glmer(urchin_presence_transect ~ kelp_presence_transect + avg_depth +(1|Year)+ (1|Month), 
             data = data, 
             family = binomial)

AIC14 <- AIC(m14)

#Model 15: Random: Year, Month, Fixed: kelp, depth, substrate
m15 <- glmer(urchin_presence_transect ~ kelp_presence_transect + avg_depth + Substrate + (1|Year) + (1|Month), 
             data = data, 
             family = binomial)

AIC15 <- AIC(m15)

#Model 16: Random: Year, Month Fixed: kelp, depth, substrate,invert
m16 <- glmer(urchin_presence_transect ~ kelp_presence_transect + avg_depth + Substrate + invert_richness_sum + (1|Year)  + (1|Month), 
             data = data, 
             family = binomial)
...
n <- glmer(urchin_presence_transect ~ kelp_presence_transect + invert_richness_sum + Substrate + avg_depth + (1|Year)+(1|Month), 
           data = data, 
           family = binomial)
n
